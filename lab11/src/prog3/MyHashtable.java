...
public class MyHashtable implements Iterable {
    ...
    private Entry getEntry(LinkedList[] current, Object key) {
        ...
        //now look for the desired Entry
        Entry e = null;
        if (current[hash] == null) return null;
        Iterator it = current[hash].iterator();
        while (it.hasNext()) {
            e = (Entry) it.next();
            if (e.key.equals(key)) {
                return e;
    ...
    private void rehash() {
        // for the compute about limit is 5 * TABLE
        tableSize = Math.min(maxLoadFactor * tableSize, tableSize * 2);
        LinkedList[] tableTemp = new LinkedList[tableSize];

        for (LinkedList current : table) {

            current.forEach(entry -> {
                Entry value = (Entry) entry;
                int hashcode = value.key.hashCode();

                int hash = hash(hashcode);
                if (tableTemp[hash] == null) {
                    tableTemp[hash] = new LinkedList();
                }
                tableTemp[hash].add(value);
            });

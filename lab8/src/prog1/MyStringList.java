...
public class MyStringList {
    ...
    public int minpos(int bottom, int top) {
        ...
        for (int i = bottom + 1; i <= top; ++i) {
            ...
            if (strArray[i].length() < m.length()) {
                m = strArray[i];
                index = i;
            } else if (strArray[i].length() == m.length()) {
                int result = strArray[i].compareTo(m);
                if (result < 0) {
                    m = strArray[i];
                    index = i;
                }
    ...
    boolean binSearchIt(String val) {
        ...
        while (true) {
            ...
            if (strArray[mid] == null) continue;
            if (val.length() - strArray[mid].length() < 0) {
                upper = mid - 1; //lower is not changed
            } else if (val.length() - strArray[mid].length() > 0) {
                lower = mid + 1;  //upper is not changed
            } else {
                if (strArray[mid].compareTo(val) > 0) { //search left
                    upper--; //lower is not changed
                } else {   //x > arr[mid], search right
                    lower++;  //upper is not changed
                }
            }
    ...
    public static void main(String[] args) {
        String[] testArr = {"big", "small", "tony", "holi","hola", "tall", "short", "round", "square", "enormous", "tiny", "gargantuan", "lilliputian", "numberless", "none", "vast", "miniscule"};
